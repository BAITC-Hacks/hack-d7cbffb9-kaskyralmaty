...
2. Реплики длиннее 1 с группируются агломеративной кластеризацией по косинусному расстоянию (порог 0,5); короткие присоединяются к ближайшей группе. Группы получают метки `SPEAKER_1..N` в порядке первого появления.
...
Порог 0,5 подобран на двух комплектных MP3: при нём на обоих совещаниях получается 5 голосов, как в эталонных протоколах; при 0,6–0,7 голоса начинают сливаться. Результат реального прогона: названы 4 из 5 голосов на каждом совещании; ведущему имя не присвоено, потому что к нему в записи не обращаются по имени. Ошибки: если в одной реплике Whisper звучат два человека, она целиком приписывается одному голосу. Точность разметки (DER) не измерялась — ручной посекундной разметки нет.
