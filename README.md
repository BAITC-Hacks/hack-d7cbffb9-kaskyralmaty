...
Кейс HackAlem, трек 8 «Инновации».
...
Измерено на реальном GPU-прогоне (воспроизводится без GPU, см. раздел о языках):

- казахская запись: WER **45% → 11%** после перераспознавания SeamlessM4T, поручения 3 из 3;
- смешанная речь: WER 36%, поручения 3 из 4, лишних нет;
- совещания №1/№2 из комплекта: 10 и 8 поручений, 5 голосов в каждом, названы 4 из 5 и 5 из 5.
...
- [Полная GPU-проверка свежего клона на Brev](docs/runs/1632-check-gpu-full.log): `check.sh gpu` с собственным vLLM, оба MP3, проверка JSON и DOCX — PASS (коммит 25cd367; после него в коде добавлены только скрипт снятия конфигурации и явная кодировка UTF-8 при чтении JSON).
...
Безопасность веб-сервиса рассчитана только на localhost в закрытом контуре: нет входа, защиты от CSRF и проверки заголовка `Host` (DNS rebinding), поэтому публиковать порт наружу нельзя. Лимит 100 МБ проверяется после приёма файла во временный каталог, тип файла заранее не проверяется. README проверен на uv 0.12.18; более старые версии uv не гарантируются.

## Потенциал развития

- **Точное «кто что сказал» без диаризации.** В закрытом контуре компания разворачивает собственную видеосвязь (например, self-hosted Jitsi Meet) с записью отдельной аудиодорожки на каждого участника. Тогда говорящий известен по дорожке, а не угадывается по голосу: ошибки разделения голосов исчезают, и весь контур (связь, запись, распознавание, протокол) остаётся внутри компании. В этом прототипе не реализовано.
- **Лицензия распознавания казахского.** SeamlessM4T распространяется под некоммерческой лицензией CC-BY-NC-4.0. Для коммерческого внедрения кандидат на замену: Omnilingual ASR от Meta (Apache-2.0, поддерживает казахский `kaz_Cyrl`); качество на наших записях не проверялось.
- Подключение к Teams/Zoom/Meet как участник-бот, автоматические напоминания ответственным по текущей дате, интеграция с СЭД, вход по ролям.
