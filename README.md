...
- Распознаёт русский, казахский и смешанную речь. Казахские фрагменты перераспознаёт отдельная модель SeamlessM4T, и ошибка на казахской записи падает примерно с 45% до 11%.
- Разделяет голоса и называет людей по тому, как к ним обращаются на совещании. Если признака нет, имя не выдумывается.
- Извлекает поручения с помощью ИИ-агента на локальной языковой модели. Агент может перечитать нужные реплики, а код проверяет каждый его ответ и возвращает на исправление, если ссылка ведёт в никуда или исполнителя нет в списке участников.
- Переводит «до пятницы» в дату и показывает статусы «в работе», «скоро срок», «просрочено».
- Записывает встречу прямо из браузера: микрофон и звук вкладки Teams, Zoom или Meet, после подтверждения, что участников предупредили.
...
Все цифры получены реальным прогоном моделей. Результаты лежат в репозитории, и цифры можно пересчитать без GPU.

| Проверка | Результат |
|---|---|
| Совещания №1 и №2 организаторов против эталонных протоколов | найдены 10 из 10 и 6 из 6 поручений эталона с тем же ответственным и сроком, лишних нет ([журнал](docs/runs/1708-reference-match.log)) |
...
| Скорость | совещание длиной от 3,5 до 4,5 минуты обрабатывается за 30-65 секунд |
...
Следующий шаг: открытый агентный фреймворк [Hermes Agent](https://github.com/NousResearch/hermes-agent) (Nous Research), подключённый к той же локальной модели в vLLM. Хаттама остаётся источником фактов, агент получает инструменты и действует.

1. **Контроль сроков.** Каждое утро агент находит поручения со статусом «скоро срок» и «просрочено» и пишет ответственному во внутреннюю почту или мессенджер, с цитатой из совещания. Руководитель получает сводку просрочек по подразделениям.
2. **Ответ исполнителя.** Сотрудник отвечает агенту «сделано», «нужен ещё день» или «это не моё». Агент обновляет статус, фиксирует перенос и передаёт спорное секретарю.
3. **Память организации.** Вопрос «что решили по Павлодарскому заводу за квартал?» получает ответ со ссылками на реплики из всех протоколов.
4. **Подготовка совещания.** Повестка из незакрытых поручений и напоминание докладчикам.
5. **СЭД.** Протокол и поручения регистрируются в системе документооборота как документ и резолюции.
6. **Процессы компании.** Шаблоны протоколов, регламенты и правила эскалации оформляются как навыки агента, ядро переписывать не нужно.

Каждое действие агента можно проверить по ссылке на исходную реплику и журналу действий. В прототипе агентного слоя пока нет. Остальные шаги (точность смешанной речи, лицензия, длинные совещания) описаны [в конце](#точность-и-масштаб).
...
Нужны Git и [uv 0.12.18](https://docs.astral.sh/uv/getting-started/installation/); Python 3.11 uv поставит сам. Из корня репозитория:
...
В `outputs/` появятся JSON и DOCX, проверка заканчивается строкой `PASS replay`. Это воспроизведение сохранённых реальных результатов, а не запуск моделей; подробности и веб-страница в разделе [«Запуск без GPU и Docker»](#запуск-без-gpu-и-docker-воспроизведение). Метрики пересчитываются так:
...
| Контроль сроков | статусы «скоро срок» и «просрочено» | есть; рассылка напоминаний в агентном слое |
...
Ниже подробный запуск, устройство и ограничения.

## Полный запуск моделей: Docker Compose и GPU
...
- [Холодный старт на новой чистой машине](docs/runs/1645-fresh-machine-cold-readme.log): команды обоих путей запуска из README строго по порядку, от установки uv до веб-страницы — все шаги успешны, около 20 минут ([замеры](docs/ENVIRONMENT.md)).
